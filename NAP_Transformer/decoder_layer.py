...
class DecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        Args:
            ...
            memory (torch.Tensor): Encoded memory (#batch, maxlen_in, size).
            memory_mask (torch.Tensor): Encoded memory mask (#batch, maxlen_in).
            cache (torch.Tensor): cached tensors. (#batch, maxlen_out - 1, size).
    ...
    def inference(self,
        ...
        # q, k, v = tgt_q, tgt, tgt
        if kv is not None:
            x = residual + self.dropout(self.self_attn(tgt_q, kv, kv, tgt_q_mask)[0])
        else:
            x = residual + self.dropout(self.self_attn(tgt_q, tgt, tgt, tgt_q_mask)[0])
        if not self.normalize_before:
            x = self.norm1(x)
